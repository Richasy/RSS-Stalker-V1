...
namespace RSS_Stalker.Pages
{
    ...
    /// </summary>
    public sealed partial class AddCustomPage : Page
    {
        ...
        public AddCustomPage()
        {
            this.InitializeComponent();
        }
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            var temp = new List<GroupChannelList>();
            foreach (var item in MainPage.Current.Categories)
            {
                temp.Add(new GroupChannelList(item.Channels) { Key = item.Name });
            }
            SourceChannelCollection = new ObservableCollection<GroupChannelList>(temp);
            if (e.Parameter != null)
            {
